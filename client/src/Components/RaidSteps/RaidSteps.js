    ...
    useEffect(() => {
        axios('/users')
            .then((res) => {
                ...
                setUser({...userData});
                const completedRaids = JSON.parse(userData.completedRaids);

                console.log(completedRaids);
            ...
            <div className={styles['modal-content']}>
                ...
                <div className={styles['progress-steps']}>
                    <ul className={styles['steps-list']}>
                        <li>
                            <span className={styles['step']}>{(user.followersCount < data.minFollowers || completed) ? <img src={ok} alt="okay icon" className={styles['done']} /> : 1}</span>
                            <p className={styles['description']}>
                                You need at least <span className={styles['divide']}>{data.minFollowers} followers</span> to participate in the raid.
                            </p>
                        </li>
                        <li>
                        ...
                        <li>
                            ...
                            <p className={styles['description']}>
                                Your reply needs to contains an least <span className={styles['divide']}>{data.minCharacters} characters.</span>
                            </p>
                        </li>
                    </ul>
                </div>
                {
                    (parseInt(user.followersCount) > parseInt(data.minFollowers) || completed) ? (
                        ...
                        <div className={!redirected ? styles['followers-state'] : styles['d-none']}>
                            <span></span>
                            <a href={data.tweeterLink} onClick={() => {
                                setRedirected(true)
                            }} target="_blank" rel="noreferrer">Start Raiding</a>
                        </div>
                    )
                }
                {(redirected && !completed) && (
                    <div className={styles['followers-state']}>
                        <span></span>
                        <button onClick={checkStatusHandler} className={styles['check-status-button']} rel="noreferrer">Check Status...</button>
                    </div>
                )}
